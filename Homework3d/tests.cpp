...
TEST(performance, 20)
{
    ...
    for (int round = 0; round < 5; round++)
    {
        ...
        cout << "QuickSort on " << N << " items: " << timer.nanoseconds() << endl;
        auto ratio = (timer.nanoseconds() / calibrated_ns);
        cout << "Ratio: " << ratio << endl;
        ASSERT(ratio < 500, "QuickSort time must be < 500x the calibration time");
        cout << endl;
...
TEST(extra_credit_stable, 10)
{
    /*
    The version of quicksort we discuss in class is not stable.
    If you write a stable version of quicksort, you get extra credit.
    */
...
TEST(extra_credit_performance_stable, 5)
{
    /*
    If your algorithm does a stable sort, you can sort arrays of uniform values efficiently.
    */
    const size_t N = 10000;
    vector<int> data(N); // N zeros
    ...
    cout << "QuickSort on " << N << " identical items: " << quick_timer.nanoseconds() << endl;
    auto ratio = (quick_timer.nanoseconds() / calibrated_ns);
    cout << "Ratio: " << ratio << endl;
    ASSERT(ratio < 4000, "QuickSort time must be < 4000x the calibration time");
}
